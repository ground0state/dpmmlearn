import numpy as np
from dpmmlearn.probability import GaussianMeanKnownVariance, InvGamma, InvGamma2D, NormInvChi2, NormInvGamma, NormInvWish
...
def test_GaussianMeanKnownVariance():
    ...
    # Test sample() method:
    prior = GaussianMeanKnownVariance(1.0, 1.0, 1.0)
    arr = prior.sample()
...
def test_InvGamma2D():
    ...
    # Test sample() method:
    prior = InvGamma2D(1.0, 1.0, np.array([0.0, 0.0]))
    arr = prior.sample()
    assert isinstance(arr, float)
...
def test_NormInvChi2():
    ...
    # Test sample() method:
    prior = NormInvChi2(1.0, 1.0, 1.0, 1.0)
    arr = prior.sample()
...
def test_NormInvGamma():
    ...
    # Test sample() method:
    prior = NormInvGamma(1.0, 1.0, 1.0, 1.0)
    arr = prior.sample()
    assert isinstance(arr, np.void)
    assert arr.dtype == prior.model_dtype
...
def test_NormInvWish():
    ...
    nu_0 = 3
    prior = NormInvWish(mu_0, kappa_0, Lam_0, nu_0)
    arr = prior.sample()
    assert isinstance(arr, np.void)
    assert arr.dtype == prior.model_dtype
