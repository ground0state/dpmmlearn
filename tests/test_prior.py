...
def test_GaussianMeanKnownVariance():
    ...
    sigsqr_0 = 1.2
    sigsqr = 0.15
    model = probability.GaussianMeanKnownVariance(mu_0, sigsqr_0, sigsqr)
    ...
    posts = [model.create_post(D)(mu) for mu in mus]
...
def test_InvGamma():
    ...
    beta = 1.2
    mu = 0.1
    ig = probability.InvGamma(alpha, beta, mu)
    ig.sample()
    ...
    posts = [ig.create_post(D)(var) for var in vars_]
...
@pytest.mark.slow
def test_InvGamma2D_slow():
    alpha = 1.1
    beta = 1.2
    mu = np.r_[0.1, 0.2]
    ig2d = probability.InvGamma2D(alpha, beta, mu)
    ig2d.sample()
...
def test_InvGamma2D():
    ...
    r = quad(ig2d.create_post(D), 0.0, np.inf)
    np.testing.assert_almost_equal(r[0], 1.0, 7,
                                   "InvGamma2D posterior density does not integrate to 1.0")

    # Check posterior predictive density
    r = dblquad(lambda x, y: ig2d.create_post(D).pred(np.r_[x, y]),
                -np.inf, np.inf,
                lambda x: -np.inf, lambda x: np.inf)
    np.testing.assert_almost_equal(
    ...
    # Check that posterior is proportional to prior * likelihood
    vars_ = [0.7, 1.1, 1.2, 1.5]
    posts = np.array([ig2d.create_post(D)(var) for var in vars_])
...
def test_NormInvChi2():
    ...
    nix = probability.NormInvChi2(mu_0, kappa_0, sigsqr_0, nu_0)
    ...
    # Check posterior density
    r = dblquad(nix.create_post(D), 0.0, np.inf, lambda x: -np.inf, lambda x: np.inf)
    np.testing.assert_almost_equal(r[0], 1.0, 7,
                                   "NormInvChi2 posterior density does not integrate to 1.0")

    # Check posterior predictive density
    r = quad(nix.create_post(D).pred, -np.inf, np.inf)
    np.testing.assert_almost_equal(
        r[0], 1.0, 10,
...
def test_NormInvGamma():
    ...
    nig = probability.NormInvGamma(m_0, V_0, a_0, b_0)
    ...
    # Check posterior density
    r = dblquad(nig.create_post(D), 0.0, np.inf, lambda x: -np.inf, lambda x: np.inf)
    np.testing.assert_almost_equal(r[0], 1.0, 7,
                                   "NormInvGamma posterior density does not integrate to 1.0")

    # Check posterior predictive density
    r = quad(nig.create_post(D).pred, -np.inf, np.inf)
    np.testing.assert_almost_equal(
        r[0], 1.0, 10,
...
def test_NormInvChi2_eq_NormInvGamma():
    ...
    model1 = probability.NormInvChi2(mu_0, kappa_0, sigsqr_0, nu_0)
    model2 = probability.NormInvGamma(m_0, V_0, a_0, b_0)
    ...
    for x in xs:
        ...
        np.testing.assert_equal(
            model1.create_post(x).pred(x), model2.create_post(x).pred(x),
    ...
    post1 = model1.create_post(xs)
    post2 = model2.create_post(xs)
    for mu, var in zip(mus, vars_):
        np.testing.assert_almost_equal(
...
def test_NormInvWish_slow():
    ...
    niw = probability.NormInvWish(mu_0, kappa_0, Lam_0, nu_0)
    ...
    niw3 = probability.NormInvWish(np.r_[1, 1, 1], 2.0, np.eye(3), 3)
...
def test_NormInvWish():
    ...
    # Check posterior predictive density
    r = dblquad(lambda x, y: niw.create_post(D).pred(np.r_[x, y]), -np.inf, np.inf,
                lambda x: -np.inf, lambda x: np.inf)
    np.testing.assert_almost_equal(
    ...
    posts = [niw.create_post(D)(mu, Sig) for mu, Sig in zip(mus, Sigs)]
